...
class LinearProjection_Concat_kv(nn.Module):
    ...
    def forward(self, x, attn_kv=None):
        ...
        kv_enc = self.to_kv(attn_kv).reshape(B_, N, 2, self.heads,
                                             C // self.heads).permute(2, 0, 3,
                                                                      1, 4)
        q, k_d, v_d = qkv_dec[0], qkv_dec[1], qkv_dec[
...
class WindowAttention(nn.Module):
    def __init__(self, dim, win_size, num_heads, token_projection='linear',
        ...
        if token_projection == 'conv':
            ...
            self.qkv = LinearProjection_Concat_kv(dim, num_heads,
                                                  dim // num_heads,
                                                  bias=qkv_bias)
        else:
    ...
    def flops(self, H, W):
        ...
        if self.token_projection != 'linear_concat':
            ...
            flops += nW * self.num_heads * N * (
                        self.dim // self.num_heads) * N * 2
            #  x = (attn @ v)
            flops += nW * self.num_heads * N * N * 2 * (
...
class LeWinTransformerBlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, win_size=8,
        ...
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim,
                       act_layer=act_layer,
                       drop=drop) if token_mlp == 'ffn' else LeFF(dim,
                                                                  mlp_hidden_dim,
                                                                  act_layer=act_layer,
                                                                  drop=drop)
...
class LeWinTransformer_Cross(nn.Module):
    ...
    def forward(self, x, attn_kv=None, mask=None):
        ...
        # cyclic shift
        if self.shift_size > 0:
            shifted_kv = torch.roll(attn_kv,
                                    shifts=(-self.shift_size, -self.shift_size),
                                    dims=(1, 2))
        else:
            shifted_kv = attn_kv
        # partition windows
        attn_kv_windows = window_partition(shifted_kv,
                                           self.win_size)  # nW*B, win_size, win_size, C
        ...
        # W-MCA/SW-MCA
        attn_windows = self.cross_attn(x_windows, attn_kv=attn_kv_windows,
                                       mask=attn_mask)  # nW*B, win_size*win_size, C
...
class BasicUformerLayer(nn.Module):
    def __init__(self, dim, output_dim, input_resolution, depth, num_heads,
                 win_size,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0.,
                 attn_drop=0.,
...
class CatCrossUformerLayer(nn.Module):
    def __init__(self, dim, output_dim, input_resolution, depth, num_heads,
        ...
        self.blocks = nn.ModuleList([
            LeWinTransformer_CatCross(dim=dim,
...
class Uformer_Cross(nn.Module):
    def __init__(self, img_size=128, in_chans=3,
        ...
        self.input_proj = InputProj(in_channel=in_chans, out_channel=embed_dim,
                                    kernel_size=3, stride=1,
                                    act_layer=nn.LeakyReLU)
        self.output_proj = OutputProj(in_channel=embed_dim,
                                      out_channel=in_chans, kernel_size=3,
                                      stride=1)
...
class Uformer_CatCross(nn.Module):
    def __init__(self, img_size=128, in_chans=3,
        ...
        enc_dpr = [x.item() for x in torch.linspace(0, drop_path_rate,
                                                    sum(depths[
                                                        :self.num_enc_layers]))]  # stochastic depth decay rule
        conv_dpr = [drop_path_rate] * depths[4]
        dec_dpr = enc_dpr[::-1]
